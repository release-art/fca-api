...
# -- Internal libraries --
import financial_services_register_api.api

from financial_services_register_api.api import FinancialServicesRegisterApiSession, FinancialServicesRegisterApiClient
...
def test_api_username():
    return os.environ['API_USERNAME']
...
class TestFinancialServicesRegisterApiClient:
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client__common_search__api_request_exception_raised(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_ref_number__resource_type_incorrect__value_error_raised(self, test_client):

        with pytest.raises(ValueError):
            await test_client._search_ref_number('resource_name', 'incorrect_resource_type')

    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_ref_number__exceptional_request__api_request_exception_raised(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_ref_number__response_not_ok__api_request_exception_raised(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_ref_number__no_fs_register_data_in_response__api_request_exception_raised(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_ref_number__fs_register_data_with_key_error__api_request_exception_raised(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_ref_number__incorrectly_specified_resource__no_fs_register_data__api_response_exception_raised(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_ref_number__inadequately_specified_resource__nonunique_fs_register_data__api_response_exception_raised(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_ref_number__correctly_and_adequately_specced_resource__unique_fs_register_data__response_returned_ok(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_frn__correctly_and_adequately_specced_firm__unique_fs_register_data__response_returned_ok(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_frn__inadequately_specced_firm__json_array_of_matching_records__response_returned_ok(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___get_resource_info__invalid_resource_type__no_modifiers__value_error_raised(self, test_client):

        with pytest.raises(ValueError):
            await test_client._get_resource_info('test_ref_number', 'invalid resource type')

    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___get_resource_info__invalid_resource_type__modifiers__value_error_raised(self, test_client):

        with pytest.raises(ValueError):
            await test_client._get_resource_info('test_ref_number', 'invalid resource type', modifiers=('test_modifier1', 'test_modifier2',))

    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___get_resource_info__no_modifiers__request_exception_raised(self, test_client):
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___get_resource_info__modifiers__request_exception_raised(self, test_client):

        with mock.patch('financial_services_register_api.api.FinancialServicesRegisterApiSession.get') as mock_api_session_get:
            mock_api_session_get.side_effect = httpx.RequestError('test RequestError')

            with pytest.raises(FinancialServicesRegisterApiRequestException):
                await test_client._get_resource_info('test_frn', 'firm', modifiers=('test_modifier1', 'test_modifier2',))
                await test_client._get_resource_info('test_prn', 'fund', modifiers=('test_modifier1', 'test_modifier2',))
                await test_client._get_resource_info('test_irn', 'individual', modifiers=('test_modifier1', 'test_modifier2',))
    ...
    async def test_financial_services_register_api_client___get_resource_info__firm(self, test_client):
        ...
        recv_response = await test_client._get_resource_info('113849', 'firm', modifiers=('Names',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the secondary or alternative business
        # or trading names of a non-existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('Names',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for the listed business
        # address of Hiscox Insurance Company Limited (FRN 113849)
        recv_response = await test_client._get_resource_info('113849', 'firm', modifiers=('Address',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the listed business address of a non-
        # existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('Address',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for the controlled functions
        # (CF) of Hiscox Insurance Company Limited (FRN 113849)
        recv_response = await test_client._get_resource_info('113849', 'firm', modifiers=('CF',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the controlled functions (CF) of a
        # non-existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('CF',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for the individuals
        # associated with Hiscox Insurance Company Limited (FRN 113849)
        recv_response = await test_client._get_resource_info('113849', 'firm', modifiers=('Individuals',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the individuals associated with a
        # existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('Individuals',))
        assert recv_response.ok
        assert not recv_response.data
        ...
        recv_response = await test_client._get_resource_info('113849', 'firm', modifiers=('Permissions',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the activities and permissions
        # associated with a non-existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('Permissions',))
        assert recv_response.ok
        assert not recv_response.data
        ...
        # non-existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('Requirements',))
        assert recv_response.ok
        assert not recv_response.data
        ...
        # Company Limited (FRN 113849)
        recv_response = await test_client._get_resource_info('113849', 'firm', modifiers=('Requirements', 'OR-0131728', 'InvestmentTypes',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for the regulators
        # listed for Hiscox Insurance Company Limited (FRN 113849)
        recv_response = await test_client._get_resource_info('113849', 'firm', modifiers=('Regulators',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the regulators listed for a
        # non-existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('Regulators',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for the passports
        # associated with Hiscox Insurance Company Limited (FRN 113849)
        recv_response = await test_client._get_resource_info('113849', 'firm', modifiers=('Passports',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the passports associated with a
        # non-existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('Passports',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for the passports
        # for a specific country, Gibraltar, associated with Hiscox Insurance Company Limited (FRN 113849)
        recv_response = await test_client._get_resource_info('113849', 'firm', modifiers=('Passports', 'Gibraltar', 'Permission',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the passports for a specific country
        # associated with a non-existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('Passports', 'Gibraltar', 'Permission',))
        assert recv_response.ok
        assert not recv_response.data
        ...
        # non-existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('Waivers',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for any exclusions
        # applying to Barclays Bank plc (FRN 122702)
        recv_response = await test_client._get_resource_info('122702', 'firm', modifiers=('Exclusions',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for any exclusions applying to a
        # non-existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('Exclusions',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for the disciplinary history
        # of Barclays Bank plc (FRN 122702)
        recv_response = await test_client._get_resource_info('122702', 'firm', modifiers=('DisciplinaryHistory',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the disciplinary history of a
        # non-existent firm
        recv_response = await test_client._get_resource_info('1234567890', 'firm', modifiers=('DisciplinaryHistory',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for the appointed representatives of
        # Hiscox Insurance Company Limited (FRN 113849)
        recv_response = await test_client._get_resource_info('113849', 'firm', modifiers=('AR',))
        assert recv_response.ok
        assert (
            recv_response.data['PreviousAppointedRepresentatives'] or 
    ...
    async def test_financial_services_register_api_client___get_resource_info__fund(self, test_client):
        ...
        recv_response = await test_client._get_resource_info('185045', 'fund', modifiers=('Subfund',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the non-existent subfund details of an
        # existing fund with PRN 1006826
        recv_response = await test_client._get_resource_info('1006826', 'fund', modifiers=('Subfund',))
        assert recv_response.ok
        assert not recv_response.data
        ...
        recv_response = await test_client._get_resource_info('185045', 'fund', modifiers=('Names',))
        assert recv_response.ok
        assert recv_response.data
        ...
        recv_response = await test_client._get_resource_info('1006826', 'fund', modifiers=('Names',))
        assert recv_response.ok
        assert not recv_response.data
        ...
        # PRN 1234567890
        recv_response = await test_client._get_resource_info('1234567890', 'fund', modifiers=('Names',))
        assert recv_response.ok
        assert not recv_response.data
    ...
    async def test_financial_services_register_api_client___get_resource_info__individual(self, test_client):
        ...
        recv_response = await test_client._get_resource_info('MXC29012', 'individual', modifiers=('CF',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of a request for the controlled functions associated
        # with a non-existent individual
        recv_response = await test_client._get_resource_info('1234567890', 'individual', modifiers=('CF',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for the disciplinary history
        # of an individual, 'Leigh Mackey' (IRN 'LXM01328')
        recv_response = await test_client._get_resource_info('LXM01328', 'individual', modifiers=('DisciplinaryHistory',))
        assert recv_response.ok
        assert recv_response.data

        # Covers the case of an unrequest for the disciplinary history
        # of an individual, 'Mark Carney' (IRN 'MXC29012')
        recv_response = await test_client._get_resource_info('MXC29012', 'individual', modifiers=('DisciplinaryHistory',))
        assert recv_response.ok
        assert not recv_response.data

        # Covers the case of a request for the disciplinary history of a non-
        # existent individual
        recv_response = await test_client._get_resource_info('1234567890', 'individual', modifiers=('DisciplinaryHistory',))
        assert recv_response.ok
        assert not recv_response.data
    ...
    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_irn__correctly_and_adequately_specced_individual__unique_fs_register_data__response_returned_ok(self, test_client):
        ...
        recv_irn = await test_client.search_irn('lynne elizabeth atkinson')
        assert isinstance(recv_irn, str)
        assert recv_irn

        recv_irn = await test_client.search_irn('margaretha jensen')
        assert isinstance(recv_irn, str)
        assert recv_irn

    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_irn__inadequately_specced_individual__json_array_of_matching_records__response_returned_ok(self, test_client):

        # Covers the case of a successful IRN search for existing, unique individuals
        recv_recs = await test_client.search_irn('john smith')
        assert isinstance(recv_recs, list)
    ...
    async def test_financial_services_register_api_client__get_individual_controlled_functions(self, test_client):
        ...
        recv_response = await test_client.get_individual_controlled_functions('1234567890')
        assert recv_response.ok
        assert not recv_response.data
    ...
    async def test_financial_services_register_api_client__get_individual_disciplinary_history(self, test_client):
        ...
        # Covers the case of a request for an non-existent individual given by
        # a non-existent IRN '1234567890'
        recv_response = await test_client.get_individual_disciplinary_history('1234567890')
        assert recv_response.ok
        assert not recv_response.data

    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_prn__correctly_and_adequately_specced_fund__unique_fs_register_data__response_returned_ok(self, test_client):
        ...
        recv_prn = await test_client.search_prn('abrdn sterling short term government bond')
        assert isinstance(recv_prn, str)
        assert recv_prn

        recv_prn = await test_client.search_prn('northern trust high dividend esg world equity feeder')
        assert isinstance(recv_prn, str)
        assert recv_prn

    @pytest.mark.asyncio
    async def test_financial_services_register_api_client___search_prn__inadequately_specced_fund__json_array_of_matching_records__response_returned_ok(self, test_client):

        # Covers the case of a successful PRN search for existing, unique funds
        recv_recs = await test_client.search_prn('jupiter')
        assert isinstance(recv_recs, list)
        assert all(isinstance(rec, dict) for rec in recv_recs)
