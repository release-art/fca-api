...
class TestFinancialServicesRegisterApiClient:
    ...
    async def test_financial_services_register_api_client___get_resource_info__firm(
        ...
        recv_response = await test_client._get_resource_info(
            "1234567890", "firm", modifiers=("Names",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        # existent firm
        recv_response = await test_client._get_resource_info(
            "1234567890", "firm", modifiers=("Address",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        recv_response = await test_client._get_resource_info(
            "113849", "firm", modifiers=("Individuals",)
        )
        assert recv_response.ok
        assert recv_response.data
        ...
        # existent firm
        recv_response = await test_client._get_resource_info(
            "1234567890", "firm", modifiers=("Individuals",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        recv_response = await test_client._get_resource_info(
            "113849", "firm", modifiers=("Permissions",)
        )
        assert recv_response.ok
        assert recv_response.data
        ...
        recv_response = await test_client._get_resource_info(
            "1234567890", "firm", modifiers=("Permissions",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        # associated with Hiscox Insurance Company Limited (FRN 113849)
        recv_response = await test_client._get_resource_info(
            "113849", "firm", modifiers=("Requirements",)
        )
        assert recv_response.ok
        assert recv_response.data
        ...
        # non-existent firm
        recv_response = await test_client._get_resource_info(
            "1234567890", "firm", modifiers=("Requirements",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        recv_response = await test_client._get_resource_info(
            "113849", "firm", modifiers=("Regulators",)
        )
        assert recv_response.ok
        assert recv_response.data
        ...
        # non-existent firm
        recv_response = await test_client._get_resource_info(
            "1234567890", "firm", modifiers=("Regulators",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        # associated with Hiscox Insurance Company Limited (FRN 113849)
        recv_response = await test_client._get_resource_info(
            "113849", "firm", modifiers=("Passports",)
        )
        assert recv_response.ok
        assert recv_response.data
        ...
        # non-existent firm
        recv_response = await test_client._get_resource_info(
            "1234567890", "firm", modifiers=("Passports",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        # non-existent firm
        recv_response = await test_client._get_resource_info(
            "1234567890", "firm", modifiers=("Waivers",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        recv_response = await test_client._get_resource_info(
            "122702", "firm", modifiers=("Exclusions",)
        )
        assert recv_response.ok
        assert recv_response.data
        ...
        # non-existent firm
        recv_response = await test_client._get_resource_info(
            "1234567890", "firm", modifiers=("Exclusions",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        recv_response = await test_client._get_resource_info(
            "122702", "firm", modifiers=("DisciplinaryHistory",)
        )
        assert recv_response.ok
        assert recv_response.data
        ...
        # non-existent firm
        recv_response = await test_client._get_resource_info(
            "1234567890", "firm", modifiers=("DisciplinaryHistory",)
        )
        assert recv_response.ok
        assert not recv_response.data
    ...
    async def test_financial_services_register_api_client___get_resource_info__fund(
        ...
        recv_response = await test_client._get_resource_info(
            "1006826", "fund", modifiers=("Subfund",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        recv_response = await test_client._get_resource_info(
            "1234567890", "fund", modifiers=("Subfund",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        # PRN 1234567890
        recv_response = await test_client._get_resource_info(
            "1234567890", "fund", modifiers=("Names",)
        )
        assert recv_response.ok
        assert not recv_response.data
    ...
    async def test_financial_services_register_api_client___get_resource_info__individual(
        ...
        recv_response = await test_client._get_resource_info(
            "MXC29012", "individual", modifiers=("CF",)
        )
        assert recv_response.ok
        assert recv_response.data
        ...
        recv_response = await test_client._get_resource_info(
            "1234567890", "individual", modifiers=("CF",)
        )
        assert recv_response.ok
        assert not recv_response.data
        ...
        recv_response = await test_client._get_resource_info(
            "LXM01328", "individual", modifiers=("DisciplinaryHistory",)
        )
        assert recv_response.ok
        assert recv_response.data
        ...
        # existent individual
        recv_response = await test_client._get_resource_info(
            "1234567890", "individual", modifiers=("DisciplinaryHistory",)
        )
        assert recv_response.ok
        assert not recv_response.data
    ...
    async def test_financial_services_register_api_client___search_prn__correctly_and_adequately_specced_fund__unique_fs_register_data__response_returned_ok(
        ...
        recv_prn = await test_client.search_prn(
            "abrdn sterling short term government bond"
        )
        assert isinstance(recv_prn, str)
        assert recv_prn

        recv_prn = await test_client.search_prn(
            "northern trust high dividend esg world equity feeder"
        )
        assert isinstance(recv_prn, str)
        assert recv_prn
